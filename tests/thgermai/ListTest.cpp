...
/*   Updated: 2021/05/26 14:48:41 by tlecoeuv         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void			compare_with_real(std::string category, ft::list<T> const &l, std::list<T> const &l2)
{
	...
	if (l.size() != l2.size())
	{
		std::cerr << WRONG << REDE << " list size isn't the same" << RESET << std::endl;
		error = true;
	}
	...
	while (!error && it != l.end())
	{
		if (*it != *it2)
		{
			std::cerr << WRONG << REDE << " list element are different : " << *it << " | " << *it2 << RESET << std::endl;
			error = true;
		}
		++it;
		++it2;
...
static void			empty_size_test()
{
	...
	if (l.size() != l2.size())
		std::cerr << WRONG << BOLD << REDE << " differents size between l and l2 : " << l.size() << " | " << l2.size() << RESET << std::endl;
	else if (l.empty() != l2.empty())
		std::cerr << WRONG << BOLD << REDE << " empty() didn't return true" << RESET << std::endl;
	else
	...
	if (l.size() != l2.size())
		...
		std::cerr << WRONG << BOLD << REDE << " empty() didn't return false" << RESET << std::endl;
	else
