...
/*   Updated: 2021/05/26 14:48:42 by tlecoeuv         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void			compare_with_real(std::string category, ft::map<T, X> const &l, std::map<T, X> const &l2)
{
	...
	if (l.size() != l2.size())
	{
		std::cerr << WRONG << REDE << " map size isn't the same" << RESET << std::endl;
		error = true;
	}
	...
	while (!error && it != l.end())
	{
		if (*it != *it2)
		{
			std::cerr << WRONG << REDE << " map element are different : " << display_it(it) << " | " << display_it(it2) << RESET << std::endl;
			error = true;
		}
		++it;
		++it2;
	...
	else
	{
		print_map(l);
		print_map(l2);
		std::cerr << WRONG << REDE << BOLD << category << " test failed" << RESET << std::endl;
	}
}
...
static void			begin_rbegin()
{
	...
	if (*it != *sit)
		std::cerr << WRONG << BOLD << REDE << " iterators value differents : " << display_it(it) << " | " << display_it(sit) << RESET << std::endl;
	else
	...
	if (*rit != *srit)
		std::cerr << WRONG << BOLD << REDE << " iterators value differents : " << display_it(rit) << " | " << display_it(srit) << RESET << std::endl;
	else
...
static void			capacity_test()
{
	...
	std::map<int, std::string>		sl;
	if (l.empty() != sl.empty())
		std::cerr << WRONG << BOLD << REDE << std::boolalpha << " Empty return error : " << l.empty() << " | " << sl.empty() << RESET << std::endl;
	else
	...
	if (l.size() != sl.size())
		std::cerr << WRONG << BOLD << REDE << " Different size : " << l.size() << " | " << sl.size() << RESET << std::endl;
	else
...
static void			operator_test()
{
	...
	for (int i = 0; i < 3; ++i)
	{
		...
		if (l[index] == sl[index])
			...
			std::cerr << WRONG << BOLD << REDE << "Values are different" << RESET << std::endl;
	}
	std::cout << std::endl;
}
	...
	for (int i = 0; i < 3; ++i)
	{
		...
		else
		{
			...
			if (*ret.first != *sret.first)
				std::cerr << WRONG << BOLD << REDE << " iterators value differents : " << display_it(ret.first) << " | " << display_it(sret.first) << RESET << std::endl;
			else
...
static void			insert_test2()
{
	...
	for (int i = 0; i < 3; ++i)
	{
		...
		if (*ret != *sret)
			std::cerr << WRONG << BOLD << REDE << " iterators value differents : " << display_it(ret) << " | " << display_it(sret) << RESET << std::endl;
...
static void			operation_test1()
{
	...
	if (res!= sres)
		std::cerr << WRONG << BOLD << REDE << " return value incorrect : " << res << " | " << sres << RESET << std::endl;
	else
