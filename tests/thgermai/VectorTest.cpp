...
/*   Updated: 2021/05/26 14:48:43 by tlecoeuv         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void			compare_with_real(std::string category, ft::vector<T> const &l, std::vector<T> const &l2)
{
	...
	if (l.size() != l2.size())
	{
		std::cerr << WRONG << REDE << " vector size isn't the same" << RESET << std::endl;
		error = true;
	}
	...
	while (!error && it != l.end())
	{
		if (*it != *it2)
		{
			std::cerr << WRONG << REDE << " vector element are different : " << *it << " | " << *it2 << RESET << std::endl;
			error = true;
		}
		++it;
		++it2;
...
static void			compare_capacity_with_real(std::string category, ft::vector<T> const &l, std::vector<T> const &l2)
{
	...
	{
		std::cerr << WRONG << REDE << " vector capacity isn't the same" << RESET << std::endl;
		error = true;
	}
...
static void			begin_rbegin_test()
{
	...
	if (*it != *sit)
		std::cerr << WRONG << BOLD << REDE << " iterators value differents : " << *it << " | " << *sit << RESET << std::endl;
	else
	...
	if (*rit != *srit)
		std::cerr << WRONG << BOLD << REDE << " iterators value differents : " << *rit << " | " << *srit << RESET << std::endl;
	else
...
static void			operator_test()
{
	...
	if (error)
		std::cerr << WRONG << REDE << BOLD << "Operator[] test failed" << RESET << std::endl;
	else
...
static void			at_test()
{
	...
	if (error)
		std::cerr << WRONG << REDE << BOLD << "at() test failed" << RESET << std::endl;
	else
	...
	{
		l.at(20);
		std::cerr << WRONG << REDE << BOLD << "at() out of index test failed" << RESET << std::endl;
	}
