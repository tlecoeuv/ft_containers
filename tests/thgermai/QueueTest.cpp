...
/*   Updated: 2021/05/26 14:47:52 by tlecoeuv         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void		empty_test()
{
	...
	if (q.empty() != rq.empty())
		error = true;
	if (error)
		std::cerr << WRONG << REDE << BOLD << "Empty() test failed" << RESET << std::endl;
	else
...
static void		size_test()
{
	...
	if (q.size() != rq.size())
		error = true;
	if (error)
		std::cerr << WRONG << REDE << BOLD << "Size() test failed" << RESET << std::endl;
	else
...
static void		back_test()
{
	...
	if (error)
		std::cerr << WRONG << REDE << BOLD << "Back() Front() test failed" << RESET << std::endl;
	else
